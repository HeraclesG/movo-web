...
const GoogleTripMapComponent = ({ route, summary, map }) => {
  ...
  const drawTrip = (tripRoute) => {
    ...
    for (let i = 0; i < tripRoute.length; i++) {
      const speed = tripRoute[i].speed
      let color;
      if (speed >= 0 && speed <= speedToKnots(55, "kmh")) {
        color = "#FD9026";
      } else if (speed > speedToKnots(55, "kmh") && speed <= speedToKnots(75, "kmh")) {
        color = "#6AAF2B";
      } else if (speed > speedToKnots(75, "kmh") && speed <= speedToKnots(90, "kmh")) {
        color = "#0054FF";
      } else if (speed > speedToKnots(90, "kmh") && speed <= speedToKnots(120, "kmh")) {
        color = "#FF01EE";
      } else if (speed > 120) {
        color = "#FF0104";
      } else {
    ...
    for (const item of route) {
      ...
      if (maxSpeed) {
        maxSpeedMarker = {
          ...
          hover: true,
          text: `${speedFromKnots(maxSpeed.speed, speedUnit).toFixed(0)} ${speedUnitString(speedUnit, t)}`,
          icon: "/max-speed.png",
        };
