...
const Alerts = ({
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await fetch(`/api/notifications?deviceId=${currentVehicleId}`, {
        method: "GET",
        headers: { "Content-Type": "application/json" },
      });
      if (response.ok) {
        const notItems = await response.json();
        ...
        {
          const item = notItems[i]
          if (item.type.toLowerCase() === "alarm") {
            if (item?.attributes?.alarms == 'accident') {
              notificationTypes.push("impactDetection")
            }
          } else {
            notificationTypes.push(item.type)
          }
        }
        const responseEvent = await fetch(`/api/reports/events?${query.toString()}`, {
          headers: { Accept: "application/json" },
        });
  
        if (responseEvent.ok) {
          const items = await responseEvent.json();
          ...
          for (let i = 0; i < reverseItems.length; i++)
          {
            ...
            if (item.type.toLowerCase() === "alarm") {
              if (item?.attributes?.alarms == 'accident') {
                item.type = "impactDetection"
              }
            }
           const notType = notificationTypes.filter((x) => x == item.type)
           console.log("real items", item)
           if (notType.length == 1) {
            realItems.push(item)
           }
          }
          setVehicleReport(realItems);
          //   }
        } else {
          throw new Error(await responseEvent.text());
        }
      } else {
  ...
  const getAlertTitle = (trip) => {
    let title = "";
    if (trip.type == "impactDetection") {
      return "Impact Detection"
    }
    else if (
